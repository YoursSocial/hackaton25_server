...
def filter_data(data, timestamp, output_file):
    ...
    """
    leo_sat_counter = 0
    iridium_sat_counter = 0
    starlink_sat_counter = 0
    ...
    for i in range(0, len(data)-3, 3):
        try: 
            ...
            #print(timestamp)
            days = ts.from_datetime(timestamp) - sat_epoch #for freshness check
            ...
            if altitude < 2000:
                ...
                if satellite.name.startswith("IRIDIUM"):
                    iridium_sat_counter += 1
                elif satellite.name.startswith("STARLINK"):
                    starlink_sat_counter += 1
    ...
    stats_data = {
        ...
        "leo_sat_counter" : leo_sat_counter,
        "iridium_sat_counter" : iridium_sat_counter,
        "starlink_sat_counter" : starlink_sat_counter
    }
    print(f"**{leo_sat_counter} LEO satellites parsed")
    return stats_data
...
if __name__ == '__main__':
    ...
    with open(output_path+output_tle, "w") as output_file, open(output_path+"stats.json", "w") as f:
        tle_date = datetime.datetime.now(datetime.UTC) #timestamp of tle_file for freshness check
